...
const stepper = (index, active) => {
  if (index + 1 < active) {
    return `flex w-full  items-center  ${
      ...
      DarkMode ? "after:border-blue-800" : "after:border-blue-400"
    }`;
  }
  return `flex w-full  items-center  ${
    DarkMode ? "text-blue-500" : "text-blue-600"
  } after:content-[''] after:w-full after:h-1 after:border-b after:border-4 after:inline-block ${
    DarkMode ? "after:border-blue-800" : "after:border-blue-100"
  }`;
};
const successIcon = () => {
  return (
    <svg
      className="w-3 h-3 text-white lg:w-3 lg:h-3 "
      aria-hidden="true"
